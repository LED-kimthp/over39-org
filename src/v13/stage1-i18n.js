...
const entryCopy = {
  ko: {
    ...
    consentHelp: "이 답변은 〈만 39세 이상〉 조사에 참여합니다. 모인 기록은 문화예술 정책을 제안하는 자료로 쓰입니다. 아래 두 가지를 확인한 뒤 시작합니다.",
    consentDetails: "저장 · 보관 · AI 사용에 관한 자세한 안내",
  ...
  en: {
    ...
    consentHelp: "This response takes part in the Over 39 study. The records gathered are used to propose arts and culture policy. Please confirm the two items below to begin.",
    consentDetails: "Details on saving, retention, and how AI is used",
  ...
  ja: {
    ...
    consentHelp: "この回答は〈39歳以上〉調査に参加します。集まった記録は文化芸術政策を提案する資料に使われます。下の二つを確認してから始めます。",
    consentDetails: "保存・保管・AI利用についての詳しい案内",
  ...
  "zh-Hans": {
    ...
    consentHelp: "这份回答将参与〈39岁以上〉调查。汇集的记录会用作提出文化艺术政策的资料。请确认下面两项后开始。",
    consentDetails: "关于保存、保管与 AI 使用的详细说明",
  ...
  "zh-Hant": {
    ...
    consentHelp: "這份回答將參與〈39歲以上〉調查。匯集的記錄會用作提出文化藝術政策的資料。請確認下面兩項後開始。",
    consentDetails: "關於儲存、保管與 AI 使用的詳細說明",
  ...
  fr: {
    ...
    consentHelp: "Cette réponse participe à l’étude « Plus de 39 ans ». Les témoignages recueillis servent à proposer des politiques pour les arts et la culture. Confirmez les deux points ci-dessous pour commencer.",
    consentDetails: "Détails sur l’enregistrement, la conservation et l’usage de l’IA",
  ...
  es: {
    ...
    consentHelp: "Esta respuesta participa en el estudio «Más de 39». Los registros reunidos se usan para proponer políticas de arte y cultura. Confirme los dos puntos siguientes para empezar.",
  ...
  nl: {
    ...
    consentHelp: "Dit antwoord neemt deel aan het onderzoek ‘39 jaar en ouder’. De verzamelde verslagen worden gebruikt om beleid voor kunst en cultuur voor te stellen. Bevestig de twee punten hieronder om te beginnen.",
    consentDetails: "Details over opslaan, bewaren en het gebruik van AI",
  ...
  ms: {
    ...
    consentHelp: "Jawapan ini menyertai kajian ‘39 Tahun ke Atas’. Rekod yang terkumpul digunakan untuk mencadangkan dasar seni dan budaya. Sila sahkan dua perkara di bawah untuk bermula.",
    consentDetails: "Butiran tentang penyimpanan, pengekalan dan penggunaan AI",
...
const task5Copy = {
  ko: { mMixed: "여러 이유가 함께 있음", mUnsure: "아직 한 가지 이유로 정하기 어려움", p16None: "현재 특별히 영향을 주는 조건이 없음", p16Unsure: "영향을 받는지 아직 모르겠음", dNoGap: "현재 특별히 큰 간극은 없음", dNoChange: "지금 특정한 변화가 필요하다고 느끼지 않음", dUnsure: "무엇이 먼저인지 아직 못 정함", noAxisMemory: "이번 기록에서는 기억의 한 방향을 정하지 않음", noAxisCondition: "이번 기록에서는 조건의 한 방향을 정하지 않음" },
  en: { mMixed: "Several reasons are present together", mUnsure: "I cannot place it in one reason yet", p16None: "No particular condition is strongly affecting this now", p16Unsure: "Not sure yet whether it affects me", dNoGap: "No major gap feels especially important now", dNoChange: "I do not currently feel a specific change is needed", dUnsure: "Not sure yet what should come first", noAxisMemory: "Leave the memory direction open in this record", noAxisCondition: "Leave the condition direction open in this record" },
  ja: { mMixed: "いくつかの理由が重なっている", mUnsure: "まだ一つの理由に決めにくい", p16None: "今とくに強く影響している条件はない", p16Unsure: "影響を受けているかまだ分からない", dNoGap: "今とくに大きな隔たりはない", dNoChange: "今は特定の変化が必要だとは感じていない", dUnsure: "何が先か、まだ決められない", noAxisMemory: "この記録では記憶の方向を一つに決めない", noAxisCondition: "この記録では条件の方向を一つに決めない" },
  "zh-Hans": { mMixed: "几个原因同时存在", mUnsure: "暂时难以归为一个原因", p16None: "目前没有特别强烈影响我的条件", p16Unsure: "还不确定是否受到影响", dNoGap: "目前没有特别明显的主要缺口", dNoChange: "目前没有感觉需要某一项具体改变", dUnsure: "还定不下什么该排在前面", noAxisMemory: "这份记录暂不确定单一记忆方向", noAxisCondition: "这份记录暂不确定单一条件方向" },
  "zh-Hant": { mMixed: "幾個原因同時存在", mUnsure: "暫時難以歸為一個原因", p16None: "目前沒有特別強烈影響我的條件", p16Unsure: "還不確定是否受到影響", dNoGap: "目前沒有特別明顯的主要缺口", dNoChange: "目前沒有感覺需要某一項具體改變", dUnsure: "還定不下什麼該排在前面", noAxisMemory: "這份記錄暫不確定單一記憶方向", noAxisCondition: "這份記錄暫不確定單一條件方向" },
  fr: { mMixed: "Plusieurs raisons sont présentes ensemble", mUnsure: "Difficile de choisir une seule raison pour l'instant", p16None: "Aucune condition particulière n'agit fortement en ce moment", p16Unsure: "Je ne sais pas encore si cela m’affecte", dNoGap: "Aucun écart majeur ne ressort actuellement", dNoChange: "Je ne ressens pas actuellement le besoin d'un changement précis", dUnsure: "Je ne sais pas encore ce qui devrait venir en premier", noAxisMemory: "Laisser ouverte la direction de la mémoire dans ce récit", noAxisCondition: "Laisser ouverte la direction des conditions dans ce récit" },
  es: { mMixed: "Hay varias razones al mismo tiempo", mUnsure: "Todavía es difícil elegir una sola razón", p16None: "Ahora no hay una condición concreta que influya especialmente", p16Unsure: "Todavía no sé si me afecta", dNoGap: "Ahora no percibo una brecha principal especialmente grande", dNoChange: "Ahora no siento que haga falta un cambio específico", dUnsure: "Todavía no sé qué debería ir primero", noAxisMemory: "Dejar abierta la dirección de la memoria en este registro", noAxisCondition: "Dejar abierta la dirección de las condiciones en este registro" },
  nl: { mMixed: "Meerdere redenen spelen tegelijk", mUnsure: "Nog moeilijk om één reden te kiezen", p16None: "Er is nu geen specifieke voorwaarde die sterk meespeelt", p16Unsure: "Nog niet zeker of het invloed heeft", dNoGap: "Er voelt nu geen bijzonder grote kloof", dNoChange: "Ik voel nu geen behoefte aan één specifieke verandering", dUnsure: "Nog niet zeker wat eerst zou moeten komen", noAxisMemory: "Laat de richting van herinnering in dit verslag open", noAxisCondition: "Laat de richting van voorwaarden in dit verslag open" },
  ms: { mMixed: "Beberapa sebab hadir bersama", mUnsure: "Masih sukar memilih satu sebab", p16None: "Tiada keadaan khusus yang sangat mempengaruhi sekarang", p16Unsure: "Masih belum pasti sama ada ia memberi kesan", dNoGap: "Tiada jurang utama yang terasa sangat besar sekarang", dNoChange: "Saya tidak merasakan perubahan khusus diperlukan sekarang", dUnsure: "Masih belum pasti apa yang perlu didahulukan", noAxisMemory: "Biarkan arah ingatan terbuka dalam rekod ini", noAxisCondition: "Biarkan arah keadaan terbuka dalam rekod ini" },
};
