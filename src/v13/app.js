import { localizeQuestion, translate } from "./i18n.js?v=v7-20260924-r87";
import { COORDINATE_SCOPE_LABELS, buildCoordinateSnapshots, deriveCoordinateScope, deriveSContextTags } from "./classification.js?v=v7-20260924-r87";
import { buildConnectionProfile, connectionTopics } from "./connection.js?v=v7-20260924-r87";
import { applicableFixedQuestionIds, buildActiveScreens, fixedQuestionIdsForScreen, flowCounts, hasSubstantiveDChange, hasSubstantiveTransition, needsContinuityQuestion, needsPauseContext, normalizedDScope, resetForRouteChange, sanitizeAnswersForRoute, withdrawAnswer } from "./flow.js?v=v7-20260924-r87";
import { ACTIVE_ANCHOR_ORDER, ADAPTIVE_POLICY_VERSION, ALL_ADAPTIVE_SCREEN_MAP, ANCHOR_AXES, ANCHOR_ORDER, aggregateAnchorSource, anchorAnswerFingerprint, anchorContextFingerprint, anchorSourceText, anchorsAffectedByChangedQuestion, assessAnchorNeed, buildAnchorContext, conditionalAnchorsAffectedByChangedQuestion, createAnchorFollowup, isLowInformationText, isStrictRealMotifPass, lowInformationReason, reconcileAnchorTurnsAfterQuestionEdit, upsertAnchorTurn, verifyDomQuestion } from "./anchor-live.js?v=v7-20260924-r87";
import { normalizeIntegratedRoleRecord, shouldShowP13Text, shouldShowP19Text, translationReuseDecision } from "./integration-r2-helpers.js?v=v7-20260924-r87";
import { ADAPTIVE_CHECKPOINTS, createClosingOffer, DEPTH_AXIS_OPTIONS, buildAdaptiveContext, buildAdaptiveSummaryContext, buildDepthTurnContext, buildMinimalDepthContext, buildMinimalSummaryContext, createAdaptiveSummary, createAdaptiveTurn, createDepthPlan, createDepthQuestion, createDepthSummary, greetingTranslationNeeded, isLiveModelSource, translateArrivedGreeting, translateResponseSummary } from "./depth.js?v=v7-20260924-r87";
import { QUESTION_METADATA } from "./question-map.js?v=v7-20260924-r87";
import { createEnvelope, readOutbox, retryOutbox, sendEnvelope, splitResearchAndContact } from "./storage.js?v=v7-20260924-r87";
import { RESPONSE_DOCUMENT_VERSION, buildResponseDocument, rawParticipantWords, renderResponseDocument, summaryParagraphsOf } from "./response-document.js?v=v7-20260924-r87";
import { GREETING_LONG_CHARS, LANGUAGE_LABELS, greetingParagraphsOf, languageLabel, narrowLanguage } from "./greeting-text.js?v=v7-20260924-r87";
import { responseDocumentFrame } from "./response-document-i18n.js?v=v7-20260924-r87";
import { compactParticipantContext, contextAwareCopy, dContextHints, hasParticipantContext, participantContextKind, participantContextOptions } from "./participant-context.js?v=v7-20260924-r87";
import { participantActivityScreenCopy, participantContextCopy } from "./participant-context-i18n.js?v=v7-20260924-r87";
import { greetingUiCopy } from "./greetings-ui-i18n.js?v=v7-20260924-r87";
import { rc2UiCopy, rc2UiPhrase } from "./rc2-ui-i18n.js?v=v7-20260924-r87";
import { completionCopy } from "./completion-i18n.js?v=v7-20260924-r87";
import { greetingVisibilityCopy, stage1ConsentCopy, stage1Copy, stage1UiExtraCopy } from "./stage1-i18n.js?v=v7-20260924-r87";
import { greetingFirstCopy } from "./greeting-first-i18n.js?v=v7-20260924-r87";
import { greetingSimplificationCopy } from "./greeting-simplification-i18n.js?v=v7-20260924-r87";
import { task7Copy } from "./task7-i18n.js?v=v7-20260924-r87";
import { POLISH_EXCLUDED_IDS, POLISH_PREFERENCE_KEY, POLISH_STATE_KEY, answerFromPolish, canPolishAgain, livePolishEntry, polishEntry, polishLeadIsShort, polishTurnedOff, rejectPolished, renderPolishExtras, renderPolishLead, requestTextPolish, withBoxEdit, withPolishFailure, withPolishResult, withPolishUse } from "./text-polish.js?v=v7-20260924-r87";
import { textPolishCopy } from "./text-polish-i18n.js?v=v7-20260924-r87";
import { createParticipantReference, publicParticipantReference } from "./participant-reference.js?v=v7-20260924-r87";
import { buildReferralBatch, parseReferralRecipients, safeReferrerLabel } from "./referral.js?v=v7-20260924-r87";
import { EXHIBITION_OPEN_CALL, buildExhibitionApplicationPayload, createDefaultExhibitionApplication, validateExhibitionApplication } from "./exhibition-application.js?v=v7-20260924-r87";
...
const buildStamp = "25bd06e938a1-dirty 2026-09-24T06:43:59.298Z";
...
// 참여자가 다듬기를 껐는가. 끄는 선택 줄은 뺐다(r84: 누르지 않으면 다듬지 않는다) — r80–r83 에서
// 끈 사람의 이어쓰기 초안만 남는다. 그런 칸에는 「껐습니다 · 다시 켜기」를 보인다.
function polishOff() {
...
// beforeInput: 칸 이름과 칸 사이에 둘 것(쓰기 전에 봐야 하는 줄임 단추 같은 것).
function renderText(id, { placeholder = "짧게 적어도 괜찮습니다.", multiline = true, label = "", field = "", value = undefined, maxChars = null, beforeInput = "" } = {}) {
  ...
  if (polishable) return `<div class="text-field text-field-polish" data-polish-field="${esc(answerField)}">${labelHtml}${renderPolishLead({ copy: textPolishCopy(state.language), esc, short: polishLeadIsShort(state.polishLeadField, answerField), off: polishOff() })}${input}${meta}${extras}</div>`;
  return `<div class="text-field">${labelHtml}${beforeInput}${input}${meta}</div>`;
}
...
// 단추는 칸 이름 바로 밑, 칸 **앞** 왼쪽에 둔다. 칸 오른쪽 아래에 있으면 다 적고 나서야 보였고,
// 「사는 곳과 같아요」 한마디로는 무엇을 하는 단추인지 몰랐다 — 옮겨 올 곳을 단추에 적는다(TK 2026-09-24).
...
function sameAsResidenceButton() {
  ...
  return `<div class="same-as-residence"><button class="secondary-button" type="button" data-action="locations-same-as-residence">${esc(t("사는 곳({place})과 같아요").replace("{place}", label))}</button></div>`;
}
...
document.addEventListener("click", (event) => {
  ...
  if (target.dataset.action === "polish-again") {
    ...
    polishTextField(target.dataset.polishId || field, field);
    return;
  }
...
async function polishTextField(id, field) {
  ...
  // 기다리는 사이에 다음 화면으로 갔으면 결과를 넣지 않는다 — 참여자가 보지 않은 문장이 답이 된다.
  // 누르고 바로 「다음」을 눌러도 쓴 글 그대로 넘어간다.
  ...
  if (result.text && !rejected && result.text.trim() !== source.trim()) {
    ...
    store.after(id);
    showPolishResult(field);
    return true;
  }
  // 고칠 것이 없었거나 실패했다. 칸에 든 글(쓴 글이든 다듬은 문장이든)은 그대로 두고 알림만 보인다.
  const current = store.entry() || withPolishFailure(null, { source, run: result.run });
  store.set({ ...current, history: [...(current.history || []), { at: new Date().toISOString(), status: "failed", source, ...(result.text && !rejected ? { error_code: "POLISH_NO_CHANGE" } : result.run?.error_code ? { error_code: result.run.error_code } : {}) }] });
...
// 다듬은 문장을 칸에 넣는다. 화면 전체를 다시 그리지 않고, 끌어 올리지도 않는다 —
// 누른 뒤 아래 질문에 답하고 있는 사람의 자리를 옮기지 않는다(TK 2026-09-24).
function showPolishResult(field) {
  const text = polishStoreFor(field).text();
  const box = document.querySelector(`textarea[data-polish-field="${CSS.escape(field)}"]`);
  if (box && box.value !== text) box.value = text;
  const counter = field === GREETING_POLISH_FIELD ? document.getElementById("greeting-message-count") : null;
  if (counter) {
    counter.textContent = `${text.length} / ${GREETING_MESSAGE_MAX}`;
    counter.classList.toggle("is-full", text.length >= GREETING_MESSAGE_MAX);
  }
  refreshPolishExtras(field);
}

// 칸에 쓰는 동안 칸 밑 단추만 켜고 끈다(두 글자 이상·다섯 번까지). 칸은 다시 그리지 않는다.
function syncPolishRunButton(field) {
  const button = document.querySelector(`.polish-extras[data-polish-extras="${CSS.escape(field)}"] [data-action="polish-again"]`);
  if (!button) return;
  const store = polishStoreFor(field);
  button.disabled = polishBusy.has(field) || !canPolishAgain(store.entry(), store.text());
}
...
document.addEventListener("input", (event) => {
  ...
  if (input.matches("[data-connection-input]")) {
    ...
    if (input.dataset.connectionInput === "message_text") {
      ...
      if (isRc2 && liveAiEnabled) syncPolishRunButton(GREETING_POLISH_FIELD);
  ...
  if (input.dataset.polishRole === "written") syncPolishRunButton(field);
